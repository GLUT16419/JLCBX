...
var js_pcb = js_pcb || {};
(function () {
	function dsn2pcb(dsn, gap, netLayersArray = []) {
		let EOF = -1;
		let stream = [dsn, 0];
		// 把数组的数组转成Map
		let netLayers = new Map(netLayersArray);
		...
		for (let structure_node of structure_root[1]) {
			if (structure_node[0] === 'layer') num_layers++;
				...
				for (let rule_node of structure_node[1]) {
					...
					} else if ((rule_node[0] === 'clear' || rule_node[0] === 'clearance') && rule_node[1].length == 1) {
		...
		for (let library_node of library_root[1]) {
			if (library_node[0] === 'image') {
				...
				// 检查 image_node[1] 是否存在且有长度
				if (library_node[1].length > 1) {
					for (let i = 1; i < library_node[1].length; ++i) {
						let image_node = library_node[1][i];
						// 确保 image_node 是数组
						if (Array.isArray(image_node) && image_node[0] === 'pin') {
			...
			if (network_node[0] == 'net') {
				for (let net_node of network_node[1]) {
					if (net_node[0] == 'pins') {
						...
						let netName = network_node[1][0][0];
						let allowedLayers = netLayers.get(netName) || null;
						the_tracks.push([net_rule[0], via_rule[0], net_rule[1], the_terminals, [], allowedLayers]);
					}
				}
			}
		}
		the_tracks.push([0.0, 0.0, 0.0, all_terminals, [], null]);
		...
		return [[Math.trunc(maxx - minx + gap * 2 + 0.5), Math.trunc(maxy - miny + gap * 2 + 0.5), num_layers], the_tracks];
	}
